<x-dashboard-layout :title="'Akun Siswa'">
    <h1 class="text-2xl font-bold mb-4 text-gray-800 dark:text-white">Akun Siswa</h1>
    <div class="grid grid-cols-1 lg:grid-cols-1 gap-6">
        <!-- ========== FORM TAMBAH SISWA KELAS ========== -->
        <div class="rounded-2xl border border-gray-200 p-5 lg:p-6 dark:border-gray-800 bg-white dark:bg-white/5">
            <h4 class="text-lg font-semibold text-gray-800 mb-6 dark:text-white/90">Update Data Siswa</h4>

            <form action="{{ route('user-siswa.update', $siswa->id) }}" method="POST">
                @method('PUT')
                @csrf
             ...
             dark:placeholder:text-white/30"
                                ...
                                <option value="aktif" {{ old('kelas', $siswa->status) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                                <option value="non-aktif" {{ old('kelas', $siswa->status) == 'non-aktif' ? 'selected' : '' }}>Non-Aktif</option>
                            </select>
                    ...
                    <div>
                        ...
                        <div x-data="{ showPassword: false }" class="relative">
                            <input name="password" :type="showPassword ? 'text' : 'password'"
                                ...
                                class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent py-2.5 pl-4 pr-11 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 
                focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 
                dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 
                dark:focus:border-brand-800" />
